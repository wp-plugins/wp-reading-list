...
get_header(); ?>
    ...
    <h1 class="entry-title" style="margin-left:<?php echo ($margin_left)?>%!important;">
    ...
    <?php if (have_posts()) {
        ...
        if ($wprl_options['layout'] == 'grid') {
            ...
            while (have_posts()) {
                ...
                <article id="post-<?php the_ID(); ?>" style="<?php _e($cssstyle);?>">
                    <header class="entry-header">
                        ...
                        if (has_post_thumbnail()) {
                            ...
                            echo ('<img src="'.esc_url($image_src[0]).'" style="width:100%;" />');
                        }
                        else{
                            echo ('<img src="'.esc_url($wprl_options['cover_image']).'"style="width:100%;">');
                        }
                        if ($worklink) {
                            echo ('</a>');
                        } ?>
                ...
                <?php
                if ($i%$width === 0 || $post_counter === (int)$wp_query->post_count) {
                    $metainfo[] = get_post(); ?>
                    <div class="spacer2">&nbsp;</div>
                    <?php foreach ($metainfo as $metapost) { ?>
                        <article id="post-<?php echo ($metapost->ID);?>" class="meta-header" style="
                        <?php _e($cssstyle);
                        if($metainfo[0] == $metapost) {
                            echo ('margin-left:'.$margin_left.'%!important;');
                        }?>
                            ">
                            <header class="entry-header">
                                <div class="entry-meta">
                                    <?php if($wprl_options['show_single_work']) { ?>
                                    <a href="<?php echo (get_permalink($metapost->ID));?>">
                                        <?php } ?>
                                        <h2 class="entry-title">
                                            <?php echo (get_the_title($metapost->ID)); ?>
                                        </h2>
                                        <?php if($wprl_options['show_single_work']) { ?>
                                    </a>
                                <?php }
                                        ...
                                        if ($typelist = get_the_terms($metapost->ID, 'work-type')) {
                                            ...
                                            foreach($typelist as $type) {
                                                ...
                                                if ($wprl_options['show_type_link']) { ?>
                                                    <a href="<?php echo (site_url());?>/reading-list/type/<?php echo ($name);?>">
                                                <?php }
                                                echo (trim($type->name));
                                                if ($wprl_options['show_type_link']) { ?>
                                                    </a>
                        ...
                        </article><!-- #post-<?php echo($metapost->ID);?> -->
                    <?php }
                    ...
                    <header class="entry-header" style=" <?php _e($liststyle);?>"><?php
                        if ($wprl_options['list_image']) {
                            ...
                            if (has_post_thumbnail()) {
                                ...
                                echo ('<img src="'.esc_url($wprl_options['cover_image']).'" width="'.$cover_width.'" height="'.$cover_height.'">');
                            }
                            if ($worklink && $wprl_options['show_url']) {
                                echo ('</a>');
                            }
                            ...
                            <tr>
                                <td>
                                    <?php if ($wprl_options['show_author']) { ?>
                                        <span id="work-author">
                                            <?php if ($authorlist = get_the_terms($post->ID, 'work-author')) {
                                                _e('By: ');
                                                $j=1;
                                                $k=0;
                                                $numItems = count($authorlist);
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($wprl_options['post_author']) { ?>
                                        <span id="post-author"><?php _e('Posted by: ', 'wp_reading_list');?> <a href="<?php echo (site_url());?>/author/<?php echo (the_author_meta('user_nicename', $post->post_author));?>"><?php echo (the_author_meta('user_nicename', $post->post_author));?></a></span>
                                    <?php } ?>
                                </td>
                            </tr>
                            <tr>
